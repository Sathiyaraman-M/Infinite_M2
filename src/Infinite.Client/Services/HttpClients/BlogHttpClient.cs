...
public class BlogHttpClient(HttpClient httpClient)
{
    ...
    {
        return await httpClient.GetFromJsonAsync<Result<FullBlogResponse>>($"api/blogs/{id}");
    }

    public async Task<IResult<FullBlogDraftResponse>> GetFullBlogDraft(string id)
    {
        return await httpClient.GetFromJsonAsync<Result<FullBlogDraftResponse>>($"api/blogs/draft/{id}");
    }
        ...
        int n = 4)
    {
        return string.IsNullOrEmpty(authorId)
            ? await httpClient.GetFromJsonAsync<Result<List<MinifiedBlogResponse>>>(
    ...
    {
        return await httpClient.GetFromJsonAsync<Result<List<MinifiedBlogDraftResponse>>>(
    ...
    public async Task<PaginatedResult<MinifiedBlogDraftResponse>> GetAllBlogDrafts(int pageNumber, int pageSize,
        string searchString)
    {
        return await httpClient.GetFromJsonAsync<PaginatedResult<MinifiedBlogDraftResponse>>(
    ...
    {
        var response = await httpClient.PostAsJsonAsync("api/blogs/publish", request);
        return await response.ToResult();
    }

    public async Task<IResult> SaveToDrafts(EditBlogRequest request)
    {
        var response = await httpClient.PostAsJsonAsync("api/blogs/draft", request);
        return await response.ToResult();
    }
    ...
    {
        var response = await httpClient.DeleteAsync($"api/blogs/draft/{id}");
        return await response.ToResult();
    }
}