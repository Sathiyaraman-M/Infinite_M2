...
public class ProjectHttpClient
{
    ...
    {
        return await _httpClient.GetFromJsonAsync<Result<List<ProjectAuthorResponse>>>("api/projects/authors/get");
    }

    public async Task<IResult<List<MinifiedBlogResponse>>> SearchBlogs(GetBlogsRequest request)
    {
        var response = await _httpClient.PostAsJsonAsync("api/projects/blogs/search", request);
    ...
    {
        return await _httpClient.GetFromJsonAsync<Result<FullProjectResponse>>($"api/projects/{id}");
    }

    public async Task<IResult<List<ProjectBlogResponse>>> GetProjectBlogs(string id)
    {
        return await _httpClient.GetFromJsonAsync<Result<List<ProjectBlogResponse>>>($"api/projects/{id}/blogs");
    }

    public async Task<IResult<List<string>>> GetProjectAuthors(string id)
    {
        return await _httpClient.GetFromJsonAsync<Result<List<string>>>($"api/projects/{id}/authors");
    }
    ...
    public async Task<IResult<List<MinifiedProjectResponse>>> GetRecentNProjects(string authorId = null, string exclude = null, int n = 4)
    {
        return string.IsNullOrEmpty(authorId)
            ? await _httpClient.GetFromJsonAsync<Result<List<MinifiedProjectResponse>>>(
    ...
    {
        var response = await _httpClient.PostAsJsonAsync("api/projects/publish", request);
        return await response.ToResult<string>();
    }

    public async Task<IResult<string>> SaveProjectBlogs(ProjectBlogsRequest request)
    {
        var response = await _httpClient.PostAsJsonAsync("api/projects/blogs", request);
        return await response.ToResult<string>();
    }
